...
function App() {
  ...
  const renderCompletionAnimation = () => {
    return <div className="completion-animation">🎉 Level Completed! 🎉</div>;
  };

  const renderHomeScreen = () => {
    return (
      <div
        className={`challenge-container ${rowsMoved ? "rows-moved" : ""} ${
          videoReveal ? "video-reveal" : ""
        }`}
      >
        <div className="challenge-row top-row">
          <div
            className={`challenge-item ${progressArray[0] ? "completed" : ""}`}
            id="challenge1"
            onClick={() => setViewedChallenge(1)}
          >
            <FontAwesomeIcon
          ...
          <div
            className={`challenge-item ${progressArray[1] ? "completed" : ""}`}
            id="challenge2"
            onClick={() => setViewedChallenge(2)}
          >
            <FontAwesomeIcon icon={faFingerprint} size="3x" className="icon" />
          </div>
          <div
            className={`challenge-item ${progressArray[2] ? "completed" : ""}`}
            id="challenge3"
            onClick={() => setViewedChallenge(3)}
          >
        ...
        {videoReveal && (
          <div className="video-container">
            <iframe
              ...
              height="315"
              style={{
                opacity: videoReveal ? 1 : 0,
                pointerEvents: videoReveal ? "auto" : "none",
              }}
              allowFullScreen
            ></iframe>
          </div>
        ...
        <div className="challenge-row bottom-row">
          <div
            className={`challenge-item ${progressArray[3] ? "completed" : ""}`}
            id="challenge4"
            onClick={() => setViewedChallenge(4)}
          >
            <FontAwesomeIcon icon={faMemory} size="3x" className="icon" />
          </div>
          <div
            className={`challenge-item ${progressArray[4] ? "completed" : ""}`}
            id="challenge5"
            onClick={() => setViewedChallenge(5)}
          >
    ...
    <div className="App">
      ...
      <div className="progress-container">
        ...
        <div
          ...
          style={{
            width: `${(progressArray.filter(Boolean).length / 5) * 100}%`,
          }}
        ></div>
      </div>
    </div>

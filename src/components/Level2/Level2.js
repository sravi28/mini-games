...
function Level2({ progressToNextLevel }) {
  ...
  const cipheredMessage = originalMessage
    .split("")
    .map((char) =>
      char === " "
        ? " "
  ...
  return (
    <div className="container">
      ...
      <p>
        <strong>Unlock the secret!</strong> Decode the encrypted message below
        to progress.
      </p>
      ...
      {!isSuccess && !errorMessage && (
        <button className="button" onClick={handleCompletion}>
          Decrypt and Submit
        </button>
      )}
